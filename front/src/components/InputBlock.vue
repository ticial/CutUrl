...
<template>
    <div class="input-block mt-3">
        <MessageList :input_msgs="field.errors" :type="'error'" :time="0"></MessageList>
        ...
        <div class="one-line-field shadow-focus shadow-hover lift-hover-anim:hover mt-2">
            ...
            <input :type="field.type" :id="field.name" :name="field.name" v-model="field.value"
                :placeholder="field.placeholder" @input="field.errors = null" autocomplete="on">
        </div>
        
        <div v-if="field.hint" class="hint-block mt-2">
            <p v-if="field.hint.length === 1">{{ field.hint[0] }}</p>
            <li v-else v-for="el in field.hint">{{ el }}</li>
        </div>
    </div>
</template>
...
.one-line-field {
    ...
    border-width: 2px;
    border-style: solid;
    border-color: var(--silver1-color);/* #2f3336; */
